...
???- Info "Updates"
    - Created 02/2021 
    - Updated 12/20/24 - Update 10/2025
...
Built on [Apache Calcite](https://calcite.apache.org/), Flink SQL facilitates the implementation of SQL-based streaming logic. With Flink SQL, developers work with **dynamic tables**, a concept similar to materialized views in DB, while abstracting away the stream construct from the developers. Developers may write SQL statement, or use the [Table API](https://nightlies.apache.org/flink/flink-docs-release-1.19/docs/dev/table/tableapi/), a language-integrated query API for Java, Scala and Python that enables the composition of queries using relational operators such as selection, filtering, and joining. Table API is a client SDK, once submitted to the Job Manager, the code is translated to a Directed Acyclic Graph. SQL or Table API efficiently handle both bounded and unbounded streams within a unified and highly optimized system inspired by traditional databases and SQL.
...
The optimizer and planner APIs transform SQL statements for execution across distributed nodes, leveraging the lower-level stream operator API. To write SQL, developers may package the code in a Java Table API program or use the Flink SQL client: an interactive tool for submitting SQL queries to Flink and visualizing the results. 

Streams or bounded data are mapped to Tables. The following SQL loads data from a csv file and [creates a dynamic table](https://docs.confluent.io/cloud/current/flink/reference/statements/create-table.html) in Flink. The WITH section specifies the properties to external connector to read the stream from:
...
[See the getting started](../coding/getting-started.md) to run Flink open-source locally with the Flink SQL client connected to a Job Manager and Task manager running in container. [See also SQL coding practices](../coding/flink-sql.md)
...
In Confluent (cloud or Platform for Flink), the catalog defines access to the schema registry, and the Kafka cluster, so Flink SQL engine can access topic message structure to map to Table schema and being able to retrieves records from topics, for processing.
...
The `changelog.mode` property is set up by using the `WITH ('changelog.mode' = 'upsert')` options when creating the table.
...
* Data is processed using Flink SQL statements may be distributed between multiple hosts.
