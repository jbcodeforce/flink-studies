...
| **Cart update (product catalog → open carts)** | [flink-sql/11-puzzles/cart_update](https://github.com/jbcodeforce/flink-studies/tree/master/code/flink-sql/11-puzzles/cart_update/) | Confluent Cloud: product upsert catalog + cart add/remove events → normalized `cart_line_items`, regular upsert join (not temporal), `ARRAY_AGG` rollup to `integrated_cart`. Product price/availability changes fan out to active carts. Pure SQL (no PTF). Shared deploy via [flink-sql/tools](https://github.com/jbcodeforce/flink-studies/tree/master/code/flink-sql/tools/) (`deploy_manifest.json`, `make deploy` / `make undeploy` stops DML and drops tables). Scenario inserts for price drop and availability change. |
