...
--
-- Reads d16_orders (Flink updating / upsert table) and uses the TO_CHANGELOG
...
-- before the change.
--
-- The result is appended to d16_orders_out (append-only topic).  Every row
...
--
-- PARTITION BY order_id ensures all changes for the same key arrive at the
