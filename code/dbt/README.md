...
To emulate the raw data in kafka topics, there is a python code [./airbnb_streaming/scripts/produce_to_kafka.py](./airbnb_streaming/scripts/produce_to_kafka.py) to load the csv, and create schemas in the Confluent cloud schema registry, then write the records to the different topics.
...
In the models/user_reviews/sources folder, define the 3 dbt model to deduplicate and filter the raw records. The pattern is the same for each raw table. 
