...
begin
-- route wrong records
insert into `raw_error_table` select * from qlik_cdc_output_table where (data is null and headers.operation <> 'DELETE') or (data is not null and data.id is null);
-- main dedupe
...
select
  ...
  if(headers.operation in ('UPDATE', 'DELETE'), beforeData.name, '') as rec_update_user_name,
  ...
  to_timestamp(headers.`timestamp`, 'yyyy-MM-dd''T''HH:mm:ssXXX') as tx_ts,
...
select -- last projection to reduce columns
  ...
  tx_ts,
  delete_ind,
  MD5(CONCAT_WS(',',customer_id, name, email, age)) as rec_row_hash
 from (
  select *,  ROW_NUMBER() OVER (
          PARTITION BY customerid_d, rec_crud_text, changeSequence
          ORDER
            BY ts DESC
