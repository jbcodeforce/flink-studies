...
timeout 5 bash -c 'cat < /dev/null > /dev/tcp/localhost/30000' 2>/dev/null
if [ $? -eq 0 ]; then
    echo "✅ Kafka is running on localhost:30000"
else
    echo "⚠️  Warning: Cannot connect to Kafka on localhost:30000"
...
if command -v kafka-topics &> /dev/null; then
    ...
    kafka-topics --create --topic products --bootstrap-server localhost:30000 --partitions 3 --replication-factor 1 2>/dev/null || echo "   Topic may already exist"
else
    echo "⚠️  kafka-topics command not found. You may need to create the topic manually:"
    echo "   kafka-topics --create --topic products --bootstrap-server localhost:30000 --partitions 3 --replication-factor 1"
fi
...
echo "   export KAFKA_BOOTSTRAP_SERVERS=localhost:30000"
echo ""
